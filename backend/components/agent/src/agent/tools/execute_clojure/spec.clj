    ...
    {:name "execute_clojure"
     ...
     :parameters {:type "object"
                  :properties {:code {:type "string"
                                     :description (str "要执行的 Clojure (SCI) 代码。仅限 SCI 子集：clojure.core 常用函数 + http/json/env，不可 require、不可 Java 互操作。示例：纯计算 (+ 1 2)；或 (http/post \"...\" {:body (json/write-str {:token (env/get-env \"变量名\")}) :headers {\"Content-Type\" \"application/json\"}})，环境变量名仅限：" wl "。直接使用 json/*、env/get-env，不要写 require 或引用其他库。")}}
                  :required ["code"]}}))
