  ...
  (let [wl (env-whitelist-str)]
    {:name "execute_clojure"
     :description (str "在 SCI（Small Clojure Interpreter）沙箱中执行一段 Clojure 代码并返回结果。注意：这里是 Clojure (SCI) 子集，不是完整 Clojure。\n\n"
                       ...
                       "5) stock：\n"
                       "   - 全量代码 (stock/all-stock-codes)：返回 A 股所有股票代码的序列（如 \"000001.SZ\" \"600000.SH\" ...），每日更新。某日金叉/死叉用 (stock/cross-signals-on-date stock-codes trade-date) 从本地缓存统计；(stock/ma-for-multiple-stocks codes period date-from date-to) 可得区间内 MA。\n"
                       "   - 多股票日 K (stock/get-daily-k-for-multiple-stocks stock-codes date-from date-to)。仅从本地缓存读取；若数据不足会异步补数，本次仍只返回当前缓存。返回 {:ok {:fields _ :by_ts_code _ :backfill_triggered _}} 或 {:error _}；:backfill_triggered 为 true 表示已触发异步补数。\n"
                       "   - 多股票 MA (stock/ma-for-multiple-stocks stock-codes period date-from date-to)。计算 [date-from, date-to] 区间内每个交易日的 period 日 MA。3 参数时 (stock-codes period date) 表示单日。返回 {:ok {:by_ts_code _ :backfill_triggered _}}；:backfill_triggered 为 true 表示已触发异步补数。\n"
                       "   - 某日金叉/死叉统计 (stock/cross-signals-on-date stock-codes trade-date opts)。从本地缓存读该日金叉/死叉；若该日原始数据条数不足会异步补数并算 MA/金叉写库，本次仍只返回当前缓存结果。summary 含 :backfill_triggered，为 true 表示已触发异步补数。只返回股票代码 {:ts_code _}，分页每页最多 500 条；opts :page :per-page。返回 {:summary _ :pagination _ :golden_cross _ :death_cross _}。\n"
