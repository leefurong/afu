  ...
  (let [wl (env-whitelist-str)]
    {:name "execute_clojure"
     :description (str "在 SCI（Small Clojure Interpreter）沙箱中执行一段 Clojure 代码并返回结果。注意：这里是 Clojure (SCI) 子集，不是完整 Clojure。\n\n"
                       ...
                       "   - 某日金叉/死叉统计 (stock/cross-signals-on-date stock-codes trade-date) 或 (stock/cross-signals-on-date stock-codes trade-date opts)。仅读 DB 缓存，不请求 Tushare。支持分页：不传 opts 时默认返回第一页，每页最多 50 条。分页通过 opts 指定：:page 当前页（从 1 开始，默认 1）、:per-page 每页条数（默认 50，最大 50）。返回 {:summary _ :pagination _ :golden_cross _ :death_cross _}，其中 :pagination 含 :total_pages（总页数）、:per_page（每页条数）、:current_page（当前页）、:total_golden_cross、:total_death_cross。\n"
