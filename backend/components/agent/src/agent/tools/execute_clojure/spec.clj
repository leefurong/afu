  ...
  (let [wl (env-whitelist-str)]
    {:name "execute_clojure"
     :description (str "在 SCI（Small Clojure Interpreter）沙箱中执行一段 Clojure 代码并返回结果。注意：这里是 Clojure (SCI) 子集，不是完整 Clojure。\n\n"
                       ...
                       "   - K 线：(stock/get-k stock-code dwmsy beg-date count)。stock-code 如 \"000001\"；dwmsy \"日k\"|\"周k\"|\"月k\"；beg-date YYYYMMDD；count 可选、默认 20。返回 {:ok {:fields _ :items _ :source _}} 或 {:error _}；:source 为 :cache（缓存）或 :tushare（接口）；items 为 map 的向量，每条 key 为 :ts_code、:trade_date、:close 等（与 fields 对应）。\n"
